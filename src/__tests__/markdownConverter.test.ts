...
import { pasteOptions } from './helpers/markdownConverter';
...
describe('markdownConverter', () => {
    ...
    test('processes HTML and calls turndown service', async () => {
        ...
        const { markdown: result } = await convertHtmlToMarkdown('<p>Test</p>', pasteOptions());
    ...
    test('processes HTML through DOM preprocessing when includeImages is false (defensive removals still applied)', async () => {
        const { default: TurndownService } = await import('turndown');
        await convertHtmlToMarkdown('<p>Test <img src="test.jpg"> content</p>', pasteOptions({ includeImages: false }));
        const instance = (TurndownService as unknown as Mock).mock.results[0].value as {
            remove: Mock;
            addRule: Mock;
            turndown: Mock;
    ...
    test('strips leading blank lines from output', async () => {
        ...
        const { markdown: result } = await convertHtmlToMarkdown(html, pasteOptions());

...
import { INSERT_MARKDOWN_COMMAND } from '../editorCommands';
...
describe('pasteHandler', () => {
    let mockJoplin: {
        ...
        commands: {
            execute: Mock<(command: string, args: unknown) => Promise<unknown>>;
        };
    };
    ...
    beforeEach(async () => {
        ...
        mockJoplin = {
            ...
            commands: {
                execute: vi.fn<(command: string, args: unknown) => Promise<unknown>>().mockResolvedValue(true),
            },
        };
    ...
    describe('HTML conversion scenarios', () => {
        ...
        test('passes a detected Google Docs source to the conversion pipeline', async () => {
            const html = '<p>Google Docs content</p>';
            mockJoplin.clipboard.readHtml.mockResolvedValue(html);
            mockJoplin.clipboard.availableFormats.mockResolvedValue([

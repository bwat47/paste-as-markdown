...
export async function processHtml(
    ...
    try {
        ...
        // Drop non-content UI elements that shouldn't appear in Markdown (e.g., chat UI buttons)
...
// Remove generic UI elements that are noise in Markdown exports. Currently removes <button> elements entirely
// (including their text) because these are typically UI controls like "Copy", file chips, etc.
function removeNonContentUi(body: HTMLElement): void {
    const isInCode = (el: Element): boolean => !!el.closest && !!el.closest('code, pre');
